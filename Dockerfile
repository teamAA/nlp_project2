...
ADD . /app
CMD  dvc remote modify myremote --local gdrive_service_account_json_file_path src/credentials.json ; dvc pull ; streamlit run main2.py --server.port=8080 --browser.serverAddress='0.0.0.0' --server.enableCORS=false --server.enableXsrfProtection=false --server.enableWebsocketCompression=false